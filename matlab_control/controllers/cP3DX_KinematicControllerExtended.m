function p3dx = cP3DX_KinematicControllerExtended(p3dx,pgains)
...
% For a*cos(alpha) ~= 0
if abs(pPar.alpha) < pi/2 && pPar.a > 0
    vw = (-sin(p3dx.pPos.X(6))*vx + cos(p3dx.pPos.X(6))*vy)/(pPar.a*cos(pPar.alpha));
else
...
p3dx.pSC.Ud(2) = vw;
p3dx.pSC.Ud(1) = vx*cos(p3dx.pPos.X(6)) + vy*sin(p3dx.pPos.X(6)) + pPar.a*sin(pPar.alpha)*vw;
