function mCADload(tag, modelName)
% mCADload: Load AprilTag 3D CAD model (Wavefront OBJ)
% Following the Bebop architecture pattern
 
    % Load object and materials
    pathBase = fullfile('Tag_CADdata');
    objFile  = fullfile(pathBase, modelName);
    mtlFile  = fullfile(pathBase, [modelName '.mtl']);

    tag.pCAD.obj{1} = loadawobj(objFile);
    tag.pCAD.mtl{1} = loadawmtl(mtlFile);

    % Flags alignment with your pFlag structure
    tag.pFlag.CADLoaded  = 1;
    tag.pFlag.CADCreated = 0;

    % Pre-processing: Centralize and Scale based on pPar.size
    v = tag.pCAD.obj{1}.v;
    vExt = [min(v,[],2) max(v,[],2)];
    mExt = mean(vExt, 2);
    
    % Scale vertices to match the physical tag size (meters)
    % Assuming the original OBJ is in a different scale, we normalize it
    for i = 1:3
        tag.pCAD.obj{1}.v(i,:) = (v(i,:) - mExt(i)) * (tag.pPar.size / (vExt(i,2)-vExt(i,1)));
    end
    
    % Define internal axis for lines/plotting
    len = tag.pPar.size;
    tag.pCAD.axes0 = [0 len 0 0; 0 0 len 0; 0 0 0 len; 1 1 1 1];
end
