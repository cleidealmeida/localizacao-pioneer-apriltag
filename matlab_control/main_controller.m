...
%
%   v2 (segurança para experimentos físicos):
%     - WATCHDOG de pose: se /robot_pose ficar obsoleta por mais de
%       pose_timeout_s durante a operação (queda do pose_estimator, oclusão
%       longa, rede), o robô é PARADO e o cenário degrada para odometria com
%       aviso — antes, o controle continuaria com a última pose congelada.
%     - PARADA DE EMERGÊNCIA por joystick Xbox (opcional, use_joystick_estop):
%       botão B para o robô e encerra o experimento imediatamente.
%       Requer @JoyControl no path e o toolbox Simulink 3D Animation.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
use_joystick_estop = false; % true = botão B do Xbox para o robô na hora
pose_timeout_s = 1.0;       % watchdog: idade máxima aceitável de /robot_pose
...
if use_joystick_estop
    ...
    if isempty(J)
        warning('Joystick indisponível — seguindo SEM parada de emergência manual.');
    else
        disp('Parada de emergência armada: botão B para o robô.');
    end
end
...
while toc(t) < tmax
    if toc(tp) > 0.1
        ...
        % --- 1.5 Parada de emergência por joystick (se armada) ---
        if ~isempty(J)
            J.mRead();
            if J.pDigital(2) == 1   % botão B
        ...
        if ~isempty(fusedMsg)
            % Watchdog: idade da última pose fundida
            ...
            elseif use_fused_pose
                % Pose obsoleta em pleno experimento: PARAR e degradar
                warning('WATCHDOG: /robot_pose obsoleta há %.1f s. Robô parado; degradando para odometria.', pose_age);
                cmdVelMsg.linear.x = 0; cmdVelMsg.angular.z = 0;
                send(pub_cmd_vel, cmdVelMsg);
